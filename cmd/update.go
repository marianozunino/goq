...
var updateCmd = &cobra.Command{
	...
	Long:  `Update goq to the latest version either using go install or by downloading the binary from GitHub.`,
	Run:   runUpdate,
}
...
func installedWithGo() bool {
	...
	binPath := filepath.Join(gopath, "bin", "goq")
...
func updateWithGoInstall() error {
	cmd := exec.Command("go", "install", "github.com/marianozunino/goq@latest")
...
func updateFromGitHub() error {
	...
	tmpfile, err := os.CreateTemp("", "goq*.tar.gz")
	if err != nil {
...
func extractBinary(archivePath string) ([]byte, error) {
	...
	for {
		...
		if strings.HasSuffix(header.Name, "goq") {
